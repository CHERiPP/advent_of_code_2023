...
// Unfortunately, Not my solution

lib
	.getInput()
	.then((data: string) => {
		let { map, visited } = solve(data);
		map = map.map((line, y) => {
		...
		console.log(small.flat().filter((c) => c === "I").length);
	})
	.catch((err) => {
		console.log(err, err.stack);
	});

const dic: Record<string, [string, string]> = {
	"|": ["UP", "DOWN"],
	"-": ["LEFT", "RIGHT"],
	L: ["UP", "RIGHT"],
	J: ["UP", "LEFT"],
...
function start(map: string[][], { UP, DOWN, LEFT, RIGHT }) {
	const S = [];
	...
	for (let yi = 0; yi < map.length; yi++) {
		...
		for (let xi = 0; xi < map[yi].length; xi++) {
			const [UP, DOWN, LEFT, RIGHT] = ["UP", "DOWN", "LEFT", "RIGHT"].map((x) =>
				(dic[map[yi][xi]] || ([] as string[])).includes(x) ? "#" : "."
			);
			line1.push(".", UP, ".");
...
function flood(map: string[][], x: number, y: number) {
	let visited = new Set([`${x},${y}`]);
	let queue = [{ x, y }];
	let trapped = true;
	while (queue.length > 0) {
